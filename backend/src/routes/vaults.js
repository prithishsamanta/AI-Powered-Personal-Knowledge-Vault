...
router.get('/', auth, async (req, res) => {
  try {
    const vaults = await Vault.find({ userId: req.user.userId })
      .sort({ lastModified: -1 })
      .select('title description lastModified createdAt');
...
// GET /api/vaults/:id - Get specific vault with full content
  ...
  try {
    ...
    res.status(201).json({
      ...
      vault: {
        ...
        lastModified: newVault.lastModified
      }
    });
...
// PUT /api/vaults/:id - Update vault content (notes/summary)
